...
class ClientApi extends AbstractRestApi
{
    /**
     * @var array
     */
     ...
     * @inerhitDoc
     */
    public function setService(BaseService $service): static
    {
    ...
    private function getFromCacheContainer(string $key): mixed
    {
        if (strlen($key) && isset($this->cacheContainer[$key])) {
            if (getenv('APSIS_DEVELOPER')) {
                $this->service->debug('API response from cache container.', ['API_METHOD.PARAMS' => $key]);
            }
